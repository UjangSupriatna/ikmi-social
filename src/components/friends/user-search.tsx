...
export function UserSearch({ onUserSelect, placeholder = "Cari pengguna...", className }: UserSearchProps) {
  ...
  const handleSendRequest = async (userId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: 'Error', description: error.error || 'Gagal mengirim permintaan', variant: 'destructive' })
      }
    } catch {
      toast({ title: 'Error', description: 'Gagal mengirim permintaan pertemanan', variant: 'destructive' })
    } finally {
    ...
    if (friendshipStatus.status === 'pending') {
      return (
        <Button size="sm" variant="outline" disabled className="gap-1">
          ...
          {friendshipStatus.isSender ? 'Terkirim' : 'Tertunda'}
        </Button>
      )
    }
          ...
          >
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {query.trim().length >= 2 ? 'Pengguna tidak ditemukan' : 'Ketik minimal 2 karakter'}
              </div>
            ) : (
