...
import type { GroupHeaderData } from './group-header'

interface EditGroupDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  group: GroupHeaderData | null
  onGroupUpdated?: (group: unknown) => void
}
...
export function EditGroupDialog({
  ...
  React.useEffect(() => {
    if (open && group) {
      ...
      setPrivacy(group.privacy as 'public' | 'private')
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Gagal mengupdate group')
      }

      const updatedGroup = await response.json()
      toast({
        title: 'Group diupdate',
        description: `"${updatedGroup.name}" berhasil diupdate`,
      })
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Gagal mengupdate group',
        variant: 'destructive',
      })
    } finally {
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <Label htmlFor="edit-name" className="text-sm font-medium">Nama Group *</Label>
              <Input
                id="edit-name"
                value={name}
