...
import { Loader2, Calendar, MapPin, Link2, Video, Globe, Pencil, Upload, X, Image as ImageIcon } from 'lucide-react'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto w-[95vw] sm:w-full p-4 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          {/* Image Upload */}
          <div className="space-y-1.5">
            <Label className="text-xs sm:text-sm">Poster/Gambar</Label>
            {formData.image ? (
              <div className="relative h-32 rounded-lg overflow-hidden border border-border">
                <img
                  src={formData.image.startsWith('blob:') ? formData.image : (getImageUrl(formData.image) || formData.image)}
                  alt="Event poster"
