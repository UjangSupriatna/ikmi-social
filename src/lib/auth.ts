...
// Get user with verification info for login check
export async function validateCredentialsWithVerification(
  identifier: string,
  password: string
): Promise<{ user: SafeUser | null; isVerified: boolean; email: string | null; name: string | null }> {
  // Find user by email or username
  const user = await db.user.findFirst({
    where: {
      OR: [
  ...
  if (!user) {
    return { user: null, isVerified: false, email: null, name: null }
  }
  ...
  // Return safe user (without password) + verification info
  const { password: _, ...safeUser } = user
  return { 
