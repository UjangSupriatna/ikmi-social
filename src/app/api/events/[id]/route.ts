...
async function deleteImageFile(imageUrl: string) {
  if (imageUrl.startsWith('/uploads/')) {
    try {
      ...
      console.log('Deleted event image:', imageUrl)
    } catch (error) {
      console.error('Failed to delete event image:', imageUrl, error)
    }
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const existingEvent = await db.event.findUnique({
      where: { id },
      select: { createdById: true, image: true }
    })
    ...
    if (existingEvent.image) {
      await deleteImageFile(existingEvent.image)
    }
