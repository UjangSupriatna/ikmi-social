...
import { 
  ...
  Image as ImageIcon, ArrowLeft, MessageCircle, ShoppingCart, Plus, Bell
} from 'lucide-react'
...
import { GroupsList, CreateGroupDialog, EditGroupDialog, GroupHeader, GroupMembers, GroupJoinRequests } from '@/components/groups'
...
type ViewType = 'feed' | 'profile' | 'friends' | 'groups' | 'messages' | 'events' | 'notifications' | 'settings' | 'ai-assistant' | 'about' | 'help' | 'privacy' | 'terms'
...
export default function IKMISocial() {
  ...
  const [groupJoinRequestCount, setGroupJoinRequestCount] = React.useState(0)
  ...
  // Fetch groups
  const fetchGroups = React.useCallback(async (searchQuery?: string) => {
    setIsLoadingGroups(true)
    try {
      const searchParam = searchQuery || groupSearchQuery
      const url = searchParam 
        ? `/api/groups?limit=20&search=${encodeURIComponent(searchParam)}`
        : '/api/groups?limit=20'
  ...
  const fetchPostById = React.useCallback(async (postId: string) => {
  ...
  // Handle URL parameters for direct navigation (e.g., shared group links)
  React.useEffect(() => {
  ...
  }, [isAuthenticated, user?.id, fetchGroupDetails])
  ...
  // Fetch notifications count on auth and poll
  React.useEffect(() => {
    if (!isAuthenticated) return
    fetchNotificationsCount()
    const interval = setInterval(fetchNotificationsCount, 30000) // Poll every 30 seconds
  ...
  React.useEffect(() => {
    ...
    const handleNavigation = async () => {
      switch (pendingNavigation.type) {
        ...
        case 'post':
          setViewingUserId(null)
          if (pendingNavigation.postId) {
            fetchPostById(pendingNavigation.postId)
          }
          break
        case 'messages':
          setViewingUserId(null)
    ...
    handleNavigation()
  }, [pendingNavigation, isAuthenticated, user?.id, viewUserProfile, fetchGroupDetails, fetchPostById, clearNavigation, toast, conversations, fetchMessages])
  ...
  const handleLogout = async () => {
    try {
      ...
      setIsAuthenticated(false)
      setCurrentView('feed')
      toast({ title: 'Keluar', description: 'Sampai jumpa!' })
    } catch {
  ...
  const handleJoinGroup = async (groupId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        if (data.joined) {
          toast({ title: 'Bergabung!', description: data.message })
        ...
        toast({ title: 'Error', description: data.error || 'Gagal bergabung dengan grup', variant: 'destructive' })
      }
    } catch {
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
                ...
                <ProfileTabs
                  tabContent={{
                    posts: profilePosts.length === 0 ? (
                      <div className="text-center py-8 text-sm text-muted-foreground">Belum ada posts</div>
                    ) : (
                      <div className="space-y-4">
                        ...
                        <CardContent className="p-4 sm:p-6">
                          {profileUser.bio ? (
                            ...
                            <div className="text-center py-6 sm:py-8 text-xs sm:text-sm text-muted-foreground">
                              Belum ada bio. Edit profil untuk menambahkannya!
                            </div>
                          )}
                        </CardContent>
                      </Card>
        ...
        {currentView === 'friends' && !viewingUserId && (
          <motion.div key="friends" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-4xl mx-auto p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              ...
              <div>
                <h2 className="text-base sm:text-lg font-semibold">Teman</h2>
                <p className="text-xs text-muted-foreground hidden sm:block">Terhubung dengan komunitas kampus</p>
              </div>
            </div>
            ...
            <Tabs value={friendsTab} onValueChange={(v) => setFriendsTab(v as 'friends' | 'requests' | 'suggestions')} className="w-full">
              <TabsList className="grid w-full grid-cols-3 mb-4 sm:mb-6 h-10 sm:h-11">
                <TabsTrigger value="friends" className="text-xs sm:text-sm">Teman</TabsTrigger>
                <TabsTrigger value="requests" className="text-xs sm:text-sm">Permintaan</TabsTrigger>
                <TabsTrigger value="suggestions" className="text-xs sm:text-sm">Saran</TabsTrigger>
              </TabsList>
          ...
          <motion.div key="groups" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto p-3 sm:p-4">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-semibold">Grup</h2>
                  <p className="text-xs text-muted-foreground hidden sm:block">Terhubung dengan komunitas</p>
                </div>
              </div>
              {/* Desktop: Text button, Mobile: Icon only */}
              <Button onClick={() => setCreateGroupOpen(true)} size="sm" className="gap-1.5 h-8 sm:h-9 hidden sm:flex">
                <Plus className="size-4" />
                <span>Buat Group</span>
              </Button>
              <Button onClick={() => setCreateGroupOpen(true)} variant="ghost" size="icon" className="size-8 sm:hidden shrink-0">
                <Plus className="size-4" />
              </Button>
            </div>
        ...
        {currentView === 'groups' && selectedGroup && !viewingUserId && (
          <motion.div key="group-detail" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto p-3 sm:p-4">
            <GroupHeader
              ...
              onShare={() => { 
                const shareUrl = `${window.location.origin}/?view=groups&groupId=${selectedGroup.id}`;
                navigator.clipboard.writeText(shareUrl); 
                toast({ title: 'Link disalin', description: 'Link group berhasil disalin!' }); 
              }}
            ...
            <Tabs defaultValue="posts" className="mt-4 sm:mt-6">
              <TabsList className={`grid w-full mb-4 sm:mb-6 h-10 sm:h-11 ${selectedGroup.privacy === 'private' && (selectedGroup.isCreator || selectedGroup.memberRole === 'admin') ? 'grid-cols-3' : 'grid-cols-2'}`}>
                <TabsTrigger value="posts" className="text-xs sm:text-sm">Postingan</TabsTrigger>
                <TabsTrigger value="members" className="text-xs sm:text-sm">Anggota</TabsTrigger>
                {selectedGroup.privacy === 'private' && (selectedGroup.isCreator || selectedGroup.memberRole === 'admin') && (
                  <TabsTrigger value="requests" className="text-xs sm:text-sm relative">
                    Permintaan
                    {selectedGroup.pendingRequestCount && selectedGroup.pendingRequestCount > 0 && (
                      <span className="absolute -top-1 -right-1 size-4 rounded-full bg-destructive text-[10px] text-destructive-foreground flex items-center justify-center font-medium">
                        {selectedGroup.pendingRequestCount > 9 ? '9+' : selectedGroup.pendingRequestCount}
                      </span>
                    )}
                  </TabsTrigger>
                )}
              ...
              <TabsContent value="posts">
                ...
                {groupPosts.length === 0 ? (
                  <div className="text-center py-8 sm:py-12 text-xs sm:text-sm text-muted-foreground">
                    Belum ada posts. {selectedGroup.isMember ? 'Jadilah yang pertama posting!' : 'Bergabung dengan grup untuk mulai posting!'}
                  </div>
                ) : (
                  <div className="space-y-4">{groupPosts.map((post) => (
                    <PostCard 
                      key={post.id} 
                      post={post} 
                      currentUserId={user?.id} 
                      onPostDeleted={handleGroupPostDeleted}
                      isGroupAdmin={selectedGroup.isCreator || selectedGroup.memberRole === 'admin' || selectedGroup.memberRole === 'moderator'}
                    />
                  ))}</div>
                )}
              </TabsContent>
              ...
              {selectedGroup.privacy === 'private' && (selectedGroup.isCreator || selectedGroup.memberRole === 'admin') && (
                <TabsContent value="requests">
                  <GroupJoinRequests groupId={selectedGroup.id} isAdmin={true} />
                </TabsContent>
              )}
            </Tabs>
          </motion.div>
        ...
        {/* AI ASSISTANT VIEW */}
        {currentView === 'ai-assistant' && !viewingUserId && (
          <motion.div key="ai-assistant" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="h-[calc(100vh-60px)] md:h-[calc(100vh-20px)] p-2 sm:p-3 md:p-4">
            <div className="flex h-full max-w-3xl mx-auto bg-card rounded-lg border shadow-sm overflow-hidden">
              <AIAssistantPage />
            </div>
          </motion.div>
        )}
        ...
        {/* ABOUT VIEW */}
        {currentView === 'about' && !viewingUserId && (
        ...
        {/* HELP VIEW */}
        {currentView === 'help' && !viewingUserId && (
          <motion.div key="help" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <HelpPage />
          </motion.div>
        )}

        {/* PRIVACY VIEW */}
        {currentView === 'privacy' && !viewingUserId && (
        ...
        {/* TERMS VIEW */}
        {currentView === 'terms' && !viewingUserId && (
        ...
        {/* NOTIFICATIONS VIEW */}
        {currentView === 'notifications' && !viewingUserId && (
      ...
      {/* Post Dialog - for post navigation from notifications */}
...
type AuthView = 'login' | 'register' | 'check-email' | 'verification-success' | 'verification-error'
...
function AuthPages({ onLoginSuccess }: { onLoginSuccess: () => void }) {
  ...
  React.useEffect(() => {
    ...
    if (verification === 'success') {
      ...
      setVerificationMessage(message || 'Terjadi kesalahan')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'login') {
        ...
        if (response.ok) {
          toast({ title: 'Selamat datang kembali!', description: 'Berhasil masuk' })
          onLoginSuccess()
        } else if (data.needsVerification) {
          // User not verified - show check email page
          setRegisteredEmail(data.email)
          toast({ 
            title: 'Email belum diverifikasi', 
            description: data.message || 'Kami telah mengirim ulang email verifikasi',
            variant: 'default'
          })
          setView('check-email')
        } else {
          toast({ title: 'Gagal masuk', description: data.error || 'Kredensial tidak valid', variant: 'destructive' })
        }
        ...
        if (response.ok) {
          ...
          toast({ title: 'Gagal mendaftar', description: data.error || 'Gagal membuat akun', variant: 'destructive' })
        }
      }
    } catch {
    ...
    try {
      ...
      if (response.ok) {
        toast({ 
          ...
          description: data.message || 'Silakan cek inbox atau folder spam Anda'
        })
      } else {
        toast({ 
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-8">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
                  ...
                  <ol className="list-decimal list-inside space-y-1">
                    ...
                    <li>Cari email dari IKMI Social</li>
          ...
          {view === 'verification-success' && (
            <Card className="border-0 shadow-xl">
              <CardHeader className="text-center">
                ...
                <CardTitle className="text-2xl text-green-600 dark:text-green-400">Email Terverifikasi!</CardTitle>
                <CardDescription>
                  Selamat datang, {userName}! Akun Anda telah berhasil diverifikasi.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button 
          ...
          {view === 'verification-error' && (
            <Card className="border-0 shadow-xl">
              <CardHeader className="text-center">
                ...
                <CardTitle className="text-2xl text-red-600 dark:text-red-400">Verifikasi Gagal</CardTitle>
                <CardDescription>
                  {verificationMessage || 'Token verifikasi tidak valid atau sudah kedaluwarsa.'}
                </CardDescription>
              </CardHeader>
          ...
          {(view === 'login' || view === 'register') && (
            <Card className="border-0 shadow-xl">
              <CardHeader className="text-center">
                <CardTitle className="text-2xl">{view === 'login' ? 'Selamat Datang Kembali' : 'Buat Akun'}</CardTitle>
                <CardDescription>{view === 'login' ? 'Masuk untuk melanjutkan ke IKMI SOCIAL' : 'Bergabung dengan komunitas IKMI hari ini'}</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button type="submit" className="w-full" disabled={isLoading}>
                    {isLoading ? <Loader2 className="size-4 animate-spin mr-2" /> : view === 'login' ? <><LogOut className="size-4 mr-2" />Masuk</> : <><UserPlus className="size-4 mr-2" />Daftar</>}
                  </Button>
                </form>
                ...
                <div className="mt-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    <button type="button" onClick={() => { 
                      ...
                      setView(newView)
                      setFormData({ name: '', username: '', email: formData.email, password: '', confirmPassword: '' })
